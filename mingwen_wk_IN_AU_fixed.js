    ...
    function getAllRegionsByPriority(region) {
        ...
        const allRegions = ['US', 'SG', 'JP', 'KR', 'IN', 'AU', 'DE', 'SE', 'NL', 'FI', 'GB'];
    ...
    export default {
        async fetch(request, env, ctx) {
            try {
                ...
                let wkFromUrl = url.searchParams.get('wk');
                if (!wkFromUrl) {
                    const pathParts = url.pathname.split('/').filter(p => p);
                    const lastPart = pathParts[pathParts.length - 1] || '';
                    if (lastPart.toLowerCase().startsWith('wk=')) {
                        wkFromUrl = lastPart.slice(3);
                    }
                }
                
                let manualRegion = wkFromUrl || getConfigValue('wk', env.wk || env.WK);
            ...
            if (cp && cp.trim()) {
                ...
                const normalizedPath = url.pathname.endsWith('/') && url.pathname.length > 1 ? url.pathname.slice(0, -1) : url.pathname;
                    ...
                    if (normalizedPath === normalizedCustomPath + '/sub'
                        || normalizedPath.startsWith(normalizedCustomPath + '/sub/')) {
            ...
            const translations = {
                ...
                fa: {
                    ...
                    regionNames: {
                        ...
                        FI: '🇫🇮 فنلاند', GB: '🇬🇧 بریتانیا', IN: 'هند', AU: 'استرالیا'
                    },
    ...
    <body>
        ...
        <script>
            ...
            function generateClientLink(clientType, clientName) {
                ...
                var wkSelect = document.getElementById("wkRegion");
                var wkValue = wkSelect ? (wkSelect.value || '').trim() : '';

                var baseUrl = currentUrl.replace(/\/sub(\/.*)?$/, '');
                var subscriptionUrl = baseUrl + "/sub";

                if (wkValue) {
                    subscriptionUrl = baseUrl + "/sub/wk=" + encodeURIComponent(wkValue);
                }
